...
if (drawCanvas.getContext) {
	...
	ctx.font = "20px Arial";
	ctx.fillText("Guess A Letter", 50, 20);
}	
...
var wordBank = ["Nicki Minaj", "Taylor Swift", "Drake", "Katy Perry", "Chris Brown", "Beyonce", "Rihanna", "Adele"];
var lettersGuessed = [];
var selectedWord = [];
...
function pickAWord() {
	resetVariables();
	pick = Math.floor(Math.random() * wordBank.length);
	wordChoice = wordBank[pick].toLowerCase();
	countGuess.innerHTML = "<strong>"+remainingGuess+"</strong>";
	pos = wordBank.indexOf(wordBank[pick]);
	wordBank.splice(pos, 1);
...
document.onkeyup = function(event) {
	...
	if(lettersGuessed.indexOf(userChoice) === -1) {
	    ...
	    if(remainingGuess === 0 && selectedWord.join("") !== wordChoice) { 
	    	gameMessage.innerHTML = "<strong>Oops! Try Again.</strong>";
	    	audioFile.src = "assets/audio/Nicki_Minaj_-_Super_Bass-4JipHEz53sU.mp3";
	    	audioFile.type = "audio/wav"
	    	audioFile.play();
	    	pickAWord();
	    }
	}else {
	    displayMessage.innerHTML = "<strong>Choose another letter.</strong>";	
	}	
}	
...
function checkUserInput(userInput) {
	...
	if(selectedWord.join("") === wordChoice) {
		gameMessage.innerHTML = "<strong>Awesome!</strong>";
		audioFile.src = "assets/audio/Nicki_Minaj_-_Super_Bass-4JipHEz53sU.mp3";
      	audioFile.type = "mp3";
      	audioFile.play();
      	pickAWord();
